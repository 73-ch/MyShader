...
void main(void){
    ...
    st += vec2(.45 * time - .45, .45) * inv;
    color += vec3(box(st, vec2(.1), .01)) * p;
    st = bst + vec2(-.45, .45 * time - .45) * inv;
    color += vec3(box(st, vec2(.1), .01)) * p;
    st = bst + vec2(-.45 * time + .45, -.45) * inv;
    color += vec3(box(st, vec2(.1), .01)) * p;
    st = bst + vec2(.45, -.45 * time + .45) * inv;
    color += vec3(box(st, vec2(.1), .01)) * p;


    // color += vec3(box(st, vec2(.1), .01)) * (step(inter, pos.x) - step(inter + 1., pos.x)) * (step(inter, pos.y) - step(inter + 2., pos.y));
