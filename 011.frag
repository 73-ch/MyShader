...
void genPolygon(in float index, in vec2 st, in float time, in float velocity, in float size, in vec3 color, out vec3 result){
  ...
  result = color * vec3(1.0-smoothstep(.4,.41,d));
}
...
void main(){
  ...
  for(float i = 0.; i < 30.; i++){
      ...
      genPolygon(i, st + vec2(rnd1- 0.5, rnd2- 0.5 +mod(u_time, 4.)* rnd3*rnd2), mod(u_time, 4.), rnd1, rnd3, vec3(rnd1,rnd2,rnd3), polygon);
      color += polygon;
  }
