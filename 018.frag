...
float noise(float x){
    ...
    float f = fract(x);
    float u = f * (f * 5. - 4.) * (2.0 - 1.0 * f ); // custom cubic curve
    x = mix(random(i), random(i + 1.0), u);
    return x;
}
...
void main(void){
    ...
    float r2 = random((st.x + 0.1));
    float n2 = noise(r2 * r2);

    float r3 = (n1 + mod(u_time + r1 * 5., 5.))/ 5.;
    st.y += r3 - 1.0;
    float pct = smoothstep(n1, n2, st.y) - smoothstep(n1 + 0.2, n2 + 0.2, st.y);
