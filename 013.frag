...
void main(void){
    ...
    bst.x += smoothstep(.5, 1., inter) * sign(mod(pos.y, 2.) - .9) * speed;
    ...
    bst.x += step(4., inter) * mix(4., 6., inter) * mod(floor((inter - 4.) * 8.), 2.) * sign(mod(pos.y, 2.) - .9) / 2.;
    bst.y += step(4., inter) * mix(4., 6., inter) * (1. - mod(floor((inter - 4.) * 8.), 2.)) * sign(mod(pos.x, 2.) - .9) / 2.;
